...
5. Fixed soft CTA line (`consultation_content.SOFT_CTA_LINE`) - never LLM-generated, so the exact wording
   can never drift.
6. Format constraints enforced via prompt + hard-coded composition: Kannada only, ~100-200 words (cut down
   twice now - 300-600 original spec, then ~150-300, then this - see below), light emojis, serious/mystical/
   mature tone, no direct disaster predictions, no guaranteed-remedy claims, no attacks on religion/caste/
   party/community, "Vedavidhya" mentioned naturally once near the end of the body, 8-12 mixed Kannada/
   English hashtags (`consultation_content.select_hashtags()` - a curated pool + theme-matched tags, not
   LLM-generated, so hashtag quality/count is always consistent), and a final stronger booking CTA line
   (`consultation_content.STRONG_CTA_LINE`) as the literal last line of every post, after the hashtags.

**Post length has been cut twice from the original spec**: 300-600 words / 5 paragraphs -> ~150-300 words /
4 paragraphs (after the brand found the initial posts too long) -> ~100-200 words / 4 paragraphs (a later
further-shorten request). `_build_prompt()` in `consultation_content.py` now asks for 4 tight paragraphs
(news summary, a single combined astrology angle, personal-life connection, closing reflection) at 25-45
words each instead of the original 5 paragraphs at 60-110 words each - the separate "secondary astrology
angle" paragraph was folded into the primary-angle paragraph as an optional one-sentence mention rather
than its own paragraph. The length-check floors were lowered to match each time: the English draft is
rejected below 45 (unique) words (was 65, was 130 originally), and a Kannada body below 50 words logs a
warning but still posts (was 75, was 150 originally). Adjust the paragraph count/word-count numbers
directly in `_build_prompt()` if the target needs to change again - there's no config flag for this, since
it's expected to be tuned by feel rather than toggled per environment.
...
default (`max_chars=2400`, `max_output_tokens=900`, cut down alongside the post-length target above -
was `2400`/`1300` at the ~150-300 word stage, `4500`/`2400` originally) since Kannada script is token-heavy
and the default budget was sized for the older, shorter classical_content.py posts; without this override, longer posts would be silently
