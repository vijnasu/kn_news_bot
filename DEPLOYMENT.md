...
5. Fixed soft CTA line (`consultation_content.SOFT_CTA_LINE`) - never LLM-generated, so the exact wording
   can never drift.
6. Format constraints enforced via prompt + hard-coded composition: Kannada only, ~150-300 words (halved
   from the original 300-600 word spec per a later request - see below), light emojis, serious/mystical/
...
**Post length was halved from the original spec** (300-600 words / 5 paragraphs down to ~150-300 words /
4 paragraphs) after the brand found the initial posts too long. `_build_prompt()` in
`consultation_content.py` now asks for 4 tight paragraphs (news summary, a single combined astrology
angle, personal-life connection, closing reflection) at 35-55 words each instead of 5 paragraphs at
60-110 words each - the separate "secondary astrology angle" paragraph was folded into the primary-angle
paragraph as an optional one-sentence mention rather than its own paragraph. The length-check floors were
lowered to match: the English draft is rejected below 65 (unique) words (was 130), and a Kannada body
below 75 words logs a warning but still posts (was 150). Adjust the paragraph count/word-count numbers
...
default (`max_chars=2400`, `max_output_tokens=1300`, reduced alongside the halved post-length target
above - originally `4500`/`2400`) since Kannada script is token-heavy and the default budget was sized for
the older, shorter classical_content.py posts; without this override, longer posts would be silently
